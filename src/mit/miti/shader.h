...
std::string BASE_SHADER_PATH = "src/res/mit/";
...
namespace MITI { 
    ...
    public:
        ...
        static Shader Create(const char* vertexShaderPath, const char* fragmentShaderPath, std::string name) {
            ...
            _vs.open(std::string("src/res/mit/").append(vertexShaderPath));
            _fs.open(std::string("src/res/mit/").append(fragmentShaderPath));
            vss << _vs.rdbuf();
            fss << _fs.rdbuf();
